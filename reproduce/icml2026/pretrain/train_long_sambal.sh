#!/bin/bash
#SBATCH --job-name=train_long_sambal
#SBATCH --nodes=1
...
# Long-regime pretraining, SAMBAL arm — produces the headline SAMBAL
# checkpoint (Tables 2-6, Figures 1-3, and the LoRA base weights).
# Identical configuration to train_long_gptbert.sh apart from the corpus;
# see that script's header for the protocol — 5400 optimizer steps
# (--steps) at sequence length 128, with the learning-rate and batch-size
# schedules defined over a longer horizon. The released
# gptbert_sambal_long_ema.bin is this run's final save.
...
if [ $# -ge 1 ]; then
    ...
    NAME=gptbert_sambal_long_$(basename "$DATA_PATH" .bin)
else
    DATA_PATH="$DATA_DIR/babycosmofine_sambal/train_sambal_10M_tokenized.bin"
    NAME=gptbert_sambal_long
fi
