#!/bin/bash
#SBATCH --job-name=finetune_gptbert_pubmed
#SBATCH --nodes=1
...
# --enable_val_ppl_early_stop: Enable early stopping based on validation perplexity
# --val_ppl_early_stop_patience N: Stop after N epochs without PPL improvement (default: 10)
# --enable_syntaxgym: Enable early stopping based on SyntaxGym accuracy
# --syntaxgym_early_stop_patience N: Stop after N eval epochs without SyntaxGym improvement (default: 10)

python finetune_lora.py \
  --config_file ../configs/small.json \
  --tokenizer_path ../gpt-bert-babylm-small/tokenizer.json \
  --checkpoint ../trained_models/gptbert_babycosmofine_long_ema.bin \
  --train_path $DATA_DIR/pubmed/pubmed_abstracts_train.bin \
  --dev_path   $DATA_DIR/pubmed/pubmed_abstracts_dev.bin \
  ...
  --lora_r $LORAR \
  --lora_alpha $ALPHA \
  --out_dir ft_out/pubmed_lora_r_${LORAR}_a_${ALPHA}_gptbert \
  --train_embeddings \
